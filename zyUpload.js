	...
	$.fn.zyUpload = function (options, param) {
		...
		return this.each(function () {
			...
			this.createHtml = function () {
				...
				var html = '';
				html += '<a class="close_upload" onClick="close_upload()"></a>';
				if (para.dragDrop) {
					// 创建带有拖动的html
					...
					html += '				<div id="status_info" class="info">选中0个 文件，共0B。</div>';
					html += '				<div class="btns">';
			...
			this.funFilterEligibleFile = function (files) {
				...
				var allow = ["jpg", "jpeg", "bmp", "pdf", "gif", "png", "avi", "mov", "mpeg", "mp4", "mpg", "rmvb", "rm", "dat", "ts", "flv", "wmv", "mp3", "wma", "wav"];
				for (var i = 0, file; file = files[i]; i++) {
					var FileExt = file.name.substring(file.name.lastIndexOf(".") + 1).toLocaleLowerCase();
					var allowFlag = 0;
					...
					//if (file.size >= 2147483648) {//2G
					if (file.size >= 300 * 1024 * 1024) { //不能超过300MB
						alert('您这个"' + file.name + '"文件大小不能超过300MB', 'error');
					} else if (allowFlag == 0) {
						alert('您这个"' + file.name + '"文件类型不支持，素材只支持jpg,jpeg,pdf,bmp,gif,png,avi,mov,mpeg,mp4，mpg,rmvb,rm,dat,ts,flv,wmv,mp3,wma,wav等格式', 'error');
					} else {
						// 在这里需要判断当前所有文件中
						arrFiles.push(file);
					}
			...
			this.funDisposePreviewHtml = function (file, e) {
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '	<a style="height:' + para.itemHeight + ';width:' + para.itemWidth + ';" href="#" class="imgBox">';
					html += '		<div class="uploadImg" style="width:' + imgWidth + 'px">';
					html += '			<img id="uploadImage_' + file.index + '" class="upload_image" src="' + fileImgSrc + '" style="width:40px;height:40px;"  />';
					html += '		</div>';
					html += '	</a>';
					html += '	<p id="uploadProgress_' + file.index + '" class="file_progress"></p>';
					html += '	<p id="uploadFailure_' + file.index + '" class="file_failure">上传失败，请重试</p>';
			...
			this.createCorePlug = function () {
				...
				var params = {
					...
					materialNumber:para.materialNumber?para.materialNumber:8,
